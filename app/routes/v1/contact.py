...
from app.services.contact import contact_create_service, get_all_contacts_service, contact_upload_service, get_by_id_service
...
async def contact_send_data(request: ContactCreate, db: Session = Depends(get_db)):
    return contact_create_service(db, request)
...
async def all_contacts(db: Session = Depends(get_db)):
    return get_all_contacts_service(db)
...
async def contact_by_id(id: int, db: Session = Depends(get_db)):
 return get_by_id_service(id, db)
